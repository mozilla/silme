...
class JarClient(FileFormatClient):
    ...
    @classmethod
    def get_entitylist(cls, path, source=False, parser=None):
        (p, j, i) = cls._explode_path(path)
        return super(JarClient, cls).get_entitylist(path, uri=i, source=source, parser=parser)

    @classmethod
    def get_structure(cls, path, source=False, parser=None):
        (p, j, i) = cls._explode_path(path)
        return super(JarClient, cls).get_structure(path, uri=i, source=source, parser=parser)
    ...
    def get_package(cls, path,
        ...
        l10npackage = FileFormatClient.get_package(path, object_type, source, ignore)
