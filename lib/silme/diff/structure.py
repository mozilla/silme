...
from ..core.entity import is_entity
...
def get_pos_from_prev_next(prev, next):
    for i in prev:
        if is_entity(i):
            return ('after',i.id)
    for i in next:
...
class StructureDiff(list):
    ...
    def entitylistdiff(self):
        entitylistdiff = EntityListDiff(self.id)
        entitylistdiff.uri = self.uri
        entities = self.entities(type='all')
        for entity in entities.values():
            flags = entity.params['diff_flags']
            if 'modified' in flags:
                entitylistdiff.add('modified', ediff=entity, id=entity.id)
            elif 'added' in flags:
                entitylistdiff.add('added', ediff=entity, id=entity.id, pos=entity.params['diff_pos'])
            elif 'removed' in flags:
                entitylistdiff.add('removed', ediff=entity, id=entity.id)
    ...
    def entities(self, type='all'):
        (added, removed, modified) = OrderedDict(),OrderedDict(),OrderedDict()

        def _get_entities_of_type(chunks, type, context=None):
            current = added if type=='added' else removed
            opposite = added if type=='removed' else removed
            if context:
                pos = get_pos_from_prev_next(context['prev'],context['next'])
            else:
                pos = None
            for elem in chunks:
                if is_entity(elem):
        ...
        for i in self:
            if 'modified' in i['flags'] and isinstance(i['elements'][1], EntityDiff):
                ...
                _get_entities_of_type(i['elements'], 'added', i)
    ...
    for pos, elem in enumerate(structure_diff):
        if elem[0] == 'insert' and 'added' in flags:
            ...
            for i in range(0, elem[2]-elem[1]):
                ...
                elif is_entity(rec[0]):
                    if values is True:
                        entity_diff = rec[0].diff(rec[1])
